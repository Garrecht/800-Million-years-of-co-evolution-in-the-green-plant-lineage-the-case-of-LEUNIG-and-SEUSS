...
def analyze_protein_cif(file_path, output_folder, pidt_threshold, distance_cutoff):
    """
    Vizualize interacting residues between two proteins in a complex
    ...
    """
    try:
        # Extract protein names dynamically from the filename (format: fold_PROTEINA_PROTEINB_model_0.cif)
        structure_name = os.path.splitext(os.path.basename(file_path))[0]
        parts = structure_name.split("_")
        
        # Assuming the protein names are in positions 2 and 3 (e.g., fold_PROTEINA_PROTEINB_model_0)
        ...
        cmd.color("copper", "chain A and backbone")
        cmd.color("warmpink", "chain B")
        cmd.color("violet", "chain B and backbone")
        ...
        cmd.hide("everything")
        cmd.show("cartoon", "chainA_b_factor chainB_b_factor")
        cmd.show("licorice", "close_A_to_B close_B_to_A")
...
if __name__ == "__main__":
    ...
    input_folder = "cif"  # Replace with your .cif files folder
